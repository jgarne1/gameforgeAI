/*
  GameForge AI Adventure Engine v6
  ...
  - Combat is quick encounter mode: visible roaming creatures spot the pet, walk up, trigger a message, then battle begins. v6 focuses on mobile-first controls, larger gameplay space, forgiving movement, and Ember Hollow readability.
*/
(function(){
  'use strict';
  ...
  var DEFAULT_ZONE={
    id:'ember_hollow',
    name:'Ember Hollow',
    subtitle:'Guide your companion through the hollow. Use ledges to avoid wisps, open the root gate, and reach the Ember Tree.',
    width:3720,
    timeLimitMs:360000,
    ...
    platforms:[
      {id:'ledge_1',x:345,y:64,w:285,h:24,label:'solid ledge'},
      {id:'ledge_2',x:850,y:112,w:345,h:24,label:'solid ledge'},
      {id:'ledge_3',x:1410,y:72,w:320,h:24,label:'solid ledge'},
      {id:'ledge_4',x:1985,y:96,w:405,h:24,label:'solid ledge'},
      {id:'ledge_5',x:2550,y:82,w:365,h:24,label:'solid ledge'},
      {id:'ledge_6',x:3140,y:78,w:310,h:24,label:'solid ledge'}
    ],
    hazards:[
    ...
    interactables:[
      ...
      {id:'ember_tree',kind:'story',x:3445,y:0,emoji:'🌳',title:'The Ember Tree',text:'The tree pulses. Your pet steps forward, and the bark glows like it recognizes them.',discoveryId:'ember_tree'}
    ],
    ambientEvents:[
  ...
  var engine={
    ...
    creatures:[],mode:'explore',encounter:null,battle:null
  };
  ...
  function start(options){
    ...
    engine.startedAt=Date.now();engine.lastAmbientAt=Date.now();engine.lastHitAt=0;engine.lastLandingAt=0;engine.lastSightAt=0;engine.lastGroundedAt=Date.now();engine.jumpQueuedUntil=0;engine.cameraX=0;engine.mode='explore';engine.encounter=null;engine.battle=null;engine.touch=null;
  ...
  function loop(){if(!engine.running)return;if(engine.mode==='explore'){updatePhysics();updateCreatures();checkCreatureSight();checkHazards();checkCollectibles();checkGoal();maybeAmbient();}else if(engine.mode==='encounterIntro'){updateEncounterIntro();}updateCamera();updatePositions();updateCreaturePositions();updateNearest();engine.raf=requestAnimationFrame(loop);}
  ...
  function checkCreatureSight(){var now=Date.now();if(now-engine.lastSightAt<650)return;var s=engine.state;for(var i=0;i<engine.creatures.length;i++){var c=engine.creatures[i];if(c.defeated||c.state!=='roam')continue;var dx=s.x-c.x,dy=Math.abs(s.y-(c.y||0));var inFront=(c.facing>0&&dx>0)||(c.facing<0&&dx<0);var inVision=Math.abs(dx)<Number(c.vision||220)&&dy<Number(c.visionY||88);var jumpedOver=s.y>Number(c.y||0)+90;var moving=Math.abs(s.vx)>0.25||!s.onGround;if(inFront&&inVision&&!jumpedOver&&moving){engine.lastSightAt=now;beginEncounterIntro(c);break;}}}
  ...
  function winBattle(){var b=engine.battle;if(!b)return;var c=b.creature;c.defeated=true;c.state='defeated';engine.defeatedCreatures[c.id]=true;var node=engine.world.querySelector('[data-eid="'+cssEscape(c.id)+'"]');if(node)node.classList.add('defeated');engine.skillBar.classList.add('hidden');engine.battle=null;engine.encounter=null;engine.mode='explore';say((c.name||'The creature')+' calmed down and drifted away.');pop('Victory!',engine.state.x,engine.state.y+98,'good');burst(engine.state.x,GROUND_H+engine.state.y+50,'solve');updateHud();}
  ...
  function checkGoal(){if(engine.goalReached)return;if(engine.state.x>engine.zone.goalX-60){engine.goalReached=true;engine.discoveries.ember_tree=true;say('The Ember Tree recognizes your companion. Return home when ready to save the discovery.');pop('Story Discovery!',engine.state.x,engine.state.y+112,'good');burst(engine.state.x,GROUND_H+engine.state.y+66,'story');pulseWorld('story');updateHud();}}
  ...
  function updateHud(){var shards=Number(engine.collected.ember_shard||0),stumbles=Object.keys(engine.hazardsHit).reduce(function(a,k){return a+Number(engine.hazardsHit[k]||0)},0),wins=Object.keys(engine.defeatedCreatures).length;engine.hud.innerText=shards+' sparks • '+stumbles+' stumbles • '+wins+' calmed';var gateOpen=!!engine.openedGates.root_gate;if(engine.mode==='battle')engine.objective.innerText='Choose a move';else if(!gateOpen)engine.objective.innerText='Open the Root Gate';else if(!engine.goalReached)engine.objective.innerText='Reach the Ember Tree';else engine.objective.innerText='Return Home';}
