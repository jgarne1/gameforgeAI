...
(function(){
  ...
  var DEFAULT_ZONE={
    ...
    portals:[
      {id:'shaft_down',x:2790,y:0,w:126,h:138,toX:3920,toY:0,spawnOffset:185,name:'Root Shaft Descent',direction:'down',text:'The root shaft carries your pet deeper into Ember Hollow.'},
      {id:'shaft_up',x:3925,y:0,w:126,h:138,toX:2860,toY:0,spawnOffset:185,name:'Root Shaft Climb',direction:'up',text:'Your pet climbs back toward the root gate chamber.'},
      {id:'burrow_drop',x:4825,y:0,w:126,h:118,toX:4985,toY:0,spawnOffset:150,name:'Low Root Burrow',direction:'down',requiresItem:'root_claw',text:'The Root Claw opens a low burrow beneath the shaft.'},
      {id:'burrow_exit',x:5750,y:0,w:132,h:118,toX:6020,toY:0,spawnOffset:150,name:'Core Root Passage',direction:'right',text:'The burrow curls forward toward the Ember Core.'}
    ],
    collectibles:[
  ...
  function updateRoomTransition(){
    ...
    if(engine.transition.portal&&engine.transition.phase==='fade'&&age>320){
      var pt=engine.transition.portal;
      var spawn=getPortalSpawn(pt);
      engine.state.x=spawn.x;
      engine.state.y=spawn.y;
      engine.state.vx=0;engine.state.vy=0;engine.state.onGround=true;engine.lastGroundedAt=Date.now();
      engine.currentRoomId=(getRoomForX(engine.state.x)||{}).id||engine.currentRoomId;
      engine.transition.phase='arrived';
      engine.transitionCooldownUntil=Date.now()+950;
      engine.lastPortalId=pt.id||'';
      updatePortalVisuals();
    }
    if(age>860){
  ...
  function getPortalSpawn(pt){
    var offset=Number(pt.spawnOffset||170);
    var x=Number(pt.toX||engine.state.x), y=Number(pt.toY||0);
    var dir=String(pt.direction||'right');
    if(dir==='down'||dir==='right')x+=offset;
    else if(dir==='up'||dir==='left')x-=offset;
    else x+=(engine.state.facing>=0?offset:-offset);
    return {x:clamp(x,62,engine.zone.width-110),y:Math.max(0,y)};
  }
  function updatePortalVisuals(){
    if(!engine.world)return;
    Array.prototype.forEach.call(engine.world.querySelectorAll('.gfAdvPortal'),function(n){n.classList.remove('recent','near');});
    if(engine.lastPortalId){var last=engine.world.querySelector('[data-ptid="'+cssEscape(engine.lastPortalId)+'"]');if(last)last.classList.add('recent');}
  }
  ...
  function checkPortals(){
    ...
    if(Date.now()<Number(engine.transitionCooldownUntil||0)){updatePortalVisuals();return;}
    var pts=engine.zone.portals||[];
    var best=null,bestD=9999;
    for(var i=0;i<pts.length;i++){
      var pt=pts[i],px=pt.x+(pt.w||120)/2,py=Number(pt.y||0);
      var dx=Math.abs(engine.state.x-px),dy=Math.abs(engine.state.y-py);
      var inPortal=dx<Math.max(34,(pt.w||120)*.28)&&dy<98;
      var nearPortal=dx<125&&dy<115;
      var node=engine.world&&engine.world.querySelector('[data-ptid="'+cssEscape(pt.id)+'"]');
      if(node)node.classList.toggle('near',nearPortal&&!node.classList.contains('recent'));
      if(!inPortal)continue;
      if(pt.requiresItem&&!engine.keyItems[pt.requiresItem]){
        if(Date.now()-Number(engine._lastPortalHintAt||0)>900){engine._lastPortalHintAt=Date.now();say('A root passage is here, but '+friendly(pt.requiresItem)+' is needed.');}
        continue;
      }
      var d=dx+dy;
      if(d<bestD){best=pt;bestD=d;}
    }
    if(best)beginPortalTransition(best);
  }
  function beginPortalTransition(pt){
    engine.mode='roomTransition';engine.transition={portal:pt,startedAt:Date.now(),phase:'fade'};engine.transitionCooldownUntil=Date.now()+1200;engine.lastPortalId=pt.id||'';engine.state.vx=0;engine.state.vy=0;
    updatePortalVisuals();
    var destRoom=getRoomForX(Number(pt.toX||0));
    if(engine.roomOverlay){engine.roomOverlay.innerHTML='<div class="passage"><b>'+esc(pt.name||'Root Passage')+'</b><span>'+esc(pt.text||'Your pet slips through the roots.')+'</span><em>'+(destRoom?esc(destRoom.name):'')+'</em></div>';engine.roomOverlay.classList.remove('hidden');}
    var node=engine.world&&engine.world.querySelector('[data-ptid="'+cssEscape(pt.id)+'"]');if(node){node.classList.add('active');setTimeout(function(){node.classList.remove('active');},850);}
    say(pt.text||'Your pet slips through the roots.');
  }
