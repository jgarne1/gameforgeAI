/*
  GameForge AI Adventure Engine v8
  ...
  - Combat is quick encounter mode: visible roaming creatures spot the pet, walk up, trigger a message, then battle begins. v8 converts Ember Hollow into a room-based, Zelda/Metroid-inspired companion adventure with meaningful items, screen transitions, environmental consequences, and stronger story purpose.
*/
(function(){
  'use strict';
  ...
  var DEFAULT_ZONE={
    ...
    rooms:[
      {id:'outer_hollow',name:'Outer Hollow',from:0,to:1650,subtitle:"The scout trail where Mira's markers still glow.",mood:'safe'},
      {id:'root_gate',name:'Root Gate Passage',from:1650,to:3050,subtitle:'Old roots sealed the way after the first ember pulse.',mood:'mystery'},
      {id:'deep_roots',name:'Deep Roots',from:3050,to:4300,subtitle:'The hollow turns vertical, hot, and alive.',mood:'danger'},
      {id:'ember_core',name:'Ember Core',from:4300,to:5400,subtitle:'The guardian waits beneath the awakened tree.',mood:'boss'}
    ],
    keyItems:[
      {id:'ember_lantern',name:'Ember Lantern',x:1905,y:0,emoji:'🏮',text:"The lantern catches your pet's glow. Dark roots ahead now shimmer with a hidden path."},
      {id:'root_claw',name:'Root Claw',x:3650,y:126,emoji:'🪝',text:'An old root-claw tool clicks open. It can pry tangled roots apart.'}
    ],
    barriers:[
      {id:'dark_root_veil',name:'Dark Root Veil',x:2445,y:0,w:88,h:154,requiresItem:'ember_lantern',emoji:'🌘',closedText:'The path is swallowed by dark roots. Something lantern-shaped would reveal the way.',openText:'The Ember Lantern glows. The dark roots peel back and reveal the passage.'},
      {id:'claw_root_tangle',name:'Root Tangle',x:4145,y:0,w:98,h:150,requiresItem:'root_claw',emoji:'🌿',closedText:'The roots are too tight to squeeze through. A prying tool could open this.',openText:'The Root Claw hooks into the vines. The tangle snaps open.'}
    ],
    collectibles:[
    ...
    interactables:[
      {id:'trail_sign',kind:'sign',x:145,y:0,emoji:'🪧',title:'Mira’s Trail Sign',text:'Mira went ahead after the Ember Tree pulsed. Follow her markers, recover what she left behind, and find why the roots woke.'},
      {id:'mira_marker',kind:'npc',x:565,y:0,emoji:'🧭',title:'Mira’s Marker',text:'A warm trail appears in the air: Mira marked a quiet path above the wisps.',effect:'guide_trail'},
      ...
      {id:'ember_relic',kind:'relic',x:1870,y:0,emoji:'🔥',title:'Faded Ember Relic',text:'The relic answers your pet. The nearby lantern wakes, and hidden roots begin to glow.',discoveryId:'ember_relic',effect:'awaken_lantern'},
      ...
      {id:'deep_roots_marker',kind:'relic',x:3590,y:0,emoji:'🌀',title:'Deep Roots',text:'Mira’s old rope is snapped here. Something pulled her deeper. The Root Claw above may open the core path.',discoveryId:'deep_roots',effect:'root_pulse'},
      ...
      {id:'guardian_marker',kind:'story',x:4760,y:0,emoji:'🌑',title:'Guardian Ring',text:'Mira’s final mark is scorched into the stone: “It is not evil. It is guarding something afraid.”',discoveryId:'guardian_ring',effect:'guardian_whisper'},
  ...
  function mount(options){
    ...
    engine.root.innerHTML=[
      ...
      '</div>',
      '<div class="gfAdventureBottom"><span>Pet-first Adventure v8</span><span>World-first UI. Touch sides to move, swipe up to jump, use the small action bubble near objects.</span></div>'
    ].join('');
  ...
  function loop(){if(!engine.running)return;if(engine.mode==='explore'){updatePhysics();updateCreatures();checkCreatureSight();checkHazards();checkCollectibles();checkAbilityItems();checkBossTriggers();checkRoomTransition();checkGoal();maybeAmbient();}else if(engine.mode==='encounterIntro'){updateEncounterIntro();}else if(engine.mode==='roomTransition'){updateRoomTransition();}else if(engine.mode==='ending'){updateEnding();}updateCamera();updatePositions();updateCreaturePositions();updateNearest();engine.raf=requestAnimationFrame(loop);}
  ...
  function updateRoomTransition(){
    ...
    if(Date.now()-engine.transition.startedAt>720){
  ...
  function resolveAbilityBarriers(prevX){
    (engine.zone.barriers||[]).forEach(function(b){
      ...
      if(rectsOverlap(petBox.x,petBox.y,petBox.w,petBox.h,b.x,GROUND_H+b.y,b.w,b.h)){
        ...
        engine.state.vx=0;say(engine.keyItems[b.requiresItem]?'Press E / Use to open this path.':(b.closedText||'The way is blocked.'));
      }
    });
  }
  ...
  function updateNearest(){var s=engine.state,nearest=null,dist=9999;Array.prototype.forEach.call(engine.world.querySelectorAll('.gfAdvUsable'),function(node){node.classList.remove('near');});(engine.zone.interactables||[]).forEach(function(o){var d=Math.abs(s.x-o.x)+Math.abs(s.y-o.y)*.8;if(d<105&&d<dist){nearest={type:'interact',id:o.id,obj:o};dist=d;}});(engine.zone.keyItems||[]).forEach(function(k){if(engine.keyItems[k.id])return;var d=Math.abs(s.x-k.x)+Math.abs(s.y-k.y)*.8;if(d<105&&d<dist){nearest={type:'keyItem',id:k.id,obj:k};dist=d;}});
    ...
    (engine.zone.gates||[]).forEach(function(g){if(engine.openedGates[g.id])return;var d=Math.abs(s.x-(g.x+g.w/2))+Math.abs(s.y-g.y)*.8;if(d<126&&d<dist){nearest={type:'gate',id:g.id,obj:g};dist=d;}});engine.nearest=nearest;if(nearest){var sel=nearest.type==='gate'?'[data-gid="'+cssEscape(nearest.id)+'"]':(nearest.type==='barrier'?'[data-bid="'+cssEscape(nearest.id)+'"]':(nearest.type==='keyItem'?'[data-kid="'+cssEscape(nearest.id)+'"]':'[data-iid="'+cssEscape(nearest.id)+'"]'));var n=engine.world.querySelector(sel);if(n)n.classList.add('near');}
    updateTouchAction();}
  function tryInteract(){if(engine.mode!=='explore')return;if(!engine.nearest){say('Move near a glowing object, then press E.');return;}if(engine.nearest.type==='gate')tryGate(engine.nearest.id);else if(engine.nearest.type==='barrier')tryBarrier(engine.nearest.id);else if(engine.nearest.type==='keyItem')collectKeyItem(engine.nearest.id);else interact(engine.nearest.id);}
