...
(function(){
  ...
  var WORLD_JSON='/assets/worlds/whisperwind_tavern_district.json';
  var SCENE_REGISTRY_URL='/assets/worlds/world_scenes.json';
  var ASSETS={
    bg:'/assets/worlds/whisperwind_tavern_district.png',
  ...
  function start(opts){
    mount();
    opts=opts||{};
    engine.sceneId=opts.sceneId||engine.sceneId||'whisperwind_tavern_district';
  ...
  function loadSceneRegistry(done){
    ...
    fetch(SCENE_REGISTRY_URL+'?v='+(Date.now()),{cache:'no-store'}).then(function(r){return r.ok?r.json():{scenes:[]};}).then(function(j){engine.sceneRegistry=j||{scenes:[]};done();}).catch(function(){engine.sceneRegistry={scenes:[{id:'whisperwind_tavern_district',url:WORLD_JSON,name:'Whisperwind - Driftwood Tavern District'}]};done();});
  }
  ...
  function joinWorldRoom(){
    ...
    ws.send(JSON.stringify({type:'worldJoin',username:engine.username||getPlayerName(),sceneId:engine.mp.room,x:engine.state.x,y:engine.state.y,face:engine.state.face,moving:engine.state.moving,mode:engine.mode,appearance:avatarAppearance()}));
  ...
  function sendWorldPosition(force){
    ...
    ws.send(JSON.stringify({type:'worldMove',x:Math.round(engine.state.x*10)/10,y:Math.round(engine.state.y*10)/10,face:engine.state.face,moving:engine.state.moving,mode:engine.mode,appearance:avatarAppearance()}));
  }
  ...
  function upsertPeer(p,snap){
    ...
    old.id=p.id;old.username=p.username||old.username||'Wanderer';old.appearance=p.appearance||old.appearance||{};old.face=p.face||old.face||'down';old.moving=!!p.moving;old.mode=p.mode||'explore';
  ...
  function remoteAnim(p){
    var face=cardinalFace(p&&p.face);
    if(p&&p.mode==='fish')return {fps:1,frames:frames(8,4)};
    if(p&&p.moving){
      if(face==='up')return {fps:7.5,frames:frames(5,6),walk:true};
      if(face==='down')return {fps:7.5,frames:frames(4,6),walk:true};
      if(face==='left')return {fps:7.5,frames:frames(6,6),walk:true};
      return {fps:7.5,frames:frames(7,6),walk:true};
    }
    if(face==='up')return {fps:2.2,frames:frames(1,4)};
    if(face==='left')return {fps:2.2,frames:frames(2,4)};
    if(face==='right')return {fps:2.2,frames:frames(3,4)};
    return {fps:2.2,frames:frames(0,4)};
  }

  function cardinalFace(face){
    face=String(face||'down');
    if(face.indexOf('right')>=0)return 'right';
    if(face.indexOf('left')>=0)return 'left';
    if(face.indexOf('up')>=0)return 'up';
    if(face.indexOf('down')>=0)return 'down';
    return 'down';
  }
  ...
  function updateExplore(dt){
    ...
    // Face from the player's intended input first, then from velocity. This prevents the
    // common stop-frame bug where friction/collision produces a tiny reverse velocity and
    // the character snaps from right-facing to left-facing after releasing the key.
    var faceX=(Math.abs(ax)>0.12?ax:0), faceY=(Math.abs(ay)>0.12?ay:0);
    if(faceX||faceY){
      if(Math.abs(faceX)>=Math.abs(faceY)*0.85){s.face=faceX>0?'right':'left';}
      else{s.face=faceY>0?'down':'up';}
      s.lastFace=s.face;
    }else if(s.moving){
      var avx=Math.abs(s.vx), avy=Math.abs(s.vy);
      if(Math.max(avx,avy)>34){
        if(avx>=avy*.85){s.face=s.vx>0?'right':'left';}
        else{s.face=s.vy>0?'down':'up';}
        s.lastFace=s.face;
      }
    }else if(s.lastFace){
      s.face=s.lastFace;
  ...
  function updateFishing(dt){
    ...
    if(f.phase==='walk'){
      ...
      showFishing(good?'Nice! Blue tension is inside the glowing catch zone.':'Hold/release to move the blue tension bar toward the gold fish marker.','Gold marker = fish · Glowing band = catch zone · Blue bar = your tension · Progress '+Math.round(f.progress*100)+'%');updateFishHud();
  ...
  function render(dt){
    ...
    var stepBob=anim.walk?(-Math.abs(Math.sin(s.animTime*Math.PI))*1.8):0;
    sprite.style.transform='translateY('+stepBob+'px) '+(anim.flip?'scaleX(-1)':'scaleX(1)');
  ...
  function getAnim(){
    ...
    if(engine.mode==='fish'){
      if(f&&f.phase==='walk')return {key:'walk_right',fps:7.5,frames:frames(7,6),walk:true};
      // Use the dedicated fish-idle-right row when present. This keeps fishing stable while
      // avoiding the left-facing idle frame that made the character appear to turn around.
      return {key:'fish_idle_right',fps:2.2,frames:frames(8,4)};
    }
    var face=cardinalFace(st.face||st.lastFace);
    if(st.moving){
      if(face==='down')return {key:'walk_down',fps:7.5,frames:frames(4,6),walk:true};
      if(face==='up')return {key:'walk_up',fps:7.5,frames:frames(5,6),walk:true};
      if(face==='left')return {key:'walk_left',fps:7.5,frames:frames(6,6),walk:true};
      return {key:'walk_right',fps:7.5,frames:frames(7,6),walk:true};
    }
    if(face==='up')return {key:'idle_up',fps:2.2,frames:frames(1,4)};
    if(face==='left')return {key:'idle_left',fps:2.2,frames:frames(2,4)};
    if(face==='right')return {key:'idle_right',fps:2.2,frames:frames(3,4)};
  ...
  function beginReel(){var f=engine.fish;if(!f||f.phase!=='bite')return;f.phase='reel';f.t=0;f.tension=0.5;f.progress=0;f.goodTime=0;f.badTime=0;showFishing('Reel! Keep the blue tension bar inside the glowing catch zone.','Gold marker = fish · Glowing band = catch zone · Blue bar = your tension');}
...
function avatarAppearance(){try{return JSON.parse(localStorage.getItem('gf_avatar_appearance')||'{}')||{};}catch(e){return {};}}
  ...
  function updateFishHud(){var c=document.getElementById('swFishCount');if(c)c.textContent=(engine.fishStats.total||0)+' fish';var cf=document.getElementById('swCatchFill');if(cf)cf.style.width=Math.round(((engine.fish&&engine.fish.progress)||0)*100)+'%';var lv=document.getElementById('swFishLevel');if(lv)lv.textContent=engine.fishStats.level||1;var xp=document.getElementById('swFishXp');if(xp)xp.textContent=(engine.fishStats.xp||0)+'/'+(engine.fishStats.nextXp||100);updateFishingMeterVisuals();renderLogbook();}
  function updateFishingMeterVisuals(){var ui=engine.ui;if(!ui)return;var f=engine.fish||{};ui.classList.remove('phase-ready','phase-aim','phase-wait','phase-bite','phase-reel','phase-catch');if(f.phase)ui.classList.add('phase-'+f.phase);var diff=Number(f.difficulty||1);var win=Math.max(.105,.23-diff*.026);var center=Number(f.fishPos==null?0.5:f.fishPos);var left=Math.max(0,center-win);var width=Math.min(1-left,win*2);ui.style.setProperty('--fish-pos',Math.round(center*100)+'%');ui.style.setProperty('--catch-zone-left',Math.round(left*100)+'%');ui.style.setProperty('--catch-zone-width',Math.round(width*100)+'%');ui.style.setProperty('--sweet-pos',Math.round(Number(f.sweetPower||0.65)*100)+'%');}
  ...
  function renderAmbientEffects(){
    ...
    list.forEach(function(e){
      ...
      var hasSprite=!!e.sprite || cls==='waterfallCascade' || cls==='waterEdgeFoam' || cls==='waterfallMist' || cls==='waterSurfaceShimmer' || cls==='slowRiverCurrent';
      if(hasSprite){
        var defaults={waterfallCascade:'/assets/effects/waterfall_cascade_v3_sheet.png',waterEdgeFoam:'/assets/effects/waterfall_foam_v3_sheet.png',waterfallMist:'/assets/effects/waterfall_mist_v3_sheet.png',waterSurfaceShimmer:'/assets/effects/living_water_v3_sheet.png',slowRiverCurrent:'/assets/effects/river_current_v3_sheet.png'};
        var sprite=e.sprite||defaults[cls]||'/assets/effects/living_water_sheet.png';
        var defaultFps=(cls==='waterfallCascade')?6:(cls==='waterEdgeFoam')?4:(cls==='waterfallMist')?3:2;
        var frames=Math.max(1,Number(e.frames||8));
        var fps=Math.max(1,Number(e.fps||defaultFps));
        var inner=document.createElement('div');
        inner.className='swAmbientSprite '+cls+'Sprite';
        ...
        if(cls==='waterfallCascade'||cls==='waterEdgeFoam'||cls==='waterfallMist'){
          ...
          glow.className='swAmbientWaterGlow';
          d.appendChild(glow);
        }
      }
      var count=(cls==='leaves')?Math.max(8,Math.round(Number(e.density||0.45)*42)):(cls==='fireflies')?Math.max(8,Math.round(Number(e.density||0.45)*32)):(cls==='dustMotes')?Math.max(10,Math.round(Number(e.density||0.45)*38)):(cls==='waterfallMist')?Math.max(1,Math.round(Number(e.density||0.35)*3)):(cls==='waterfallCascade')?0:(cls==='waterSurfaceShimmer')?0:(cls==='slowRiverCurrent')?0:(cls==='waterEdgeFoam')?0:(cls==='pondRipple'||cls==='splashRing')?Math.max(1,Math.round(Number(e.density||0.35)*3)):0;
      for(var i=0;i<count;i++){var part=document.createElement('i');part.style.left=(Math.random()*100)+'%';part.style.top=(Math.random()*100)+'%';part.style.animationDelay=(-Math.random()*8)+'s';part.style.animationDuration=(6+Math.random()*10)/Math.max(.15,Number(e.speed||1))+'s';d.appendChild(part);}
      layer.appendChild(d);
    });
  }
...
/* Fishing meter clarity fix: makes the catch zone visible and labels the moving marker. */
.swFishing .swCastBar:before{z-index:3;pointer-events:none;transition:left .08s linear,width .08s linear,opacity .12s ease;background:rgba(34,197,94,.24)}
.swFishing.phase-aim .swCastBar:before,.swFishing.phase-ready .swCastBar:before{left:38%;right:auto;width:34%;background:linear-gradient(90deg,rgba(34,197,94,.18),rgba(255,255,255,.24),rgba(34,197,94,.18));border:2px solid rgba(255,255,255,.72);box-shadow:0 0 16px rgba(34,197,94,.42),inset 0 0 12px rgba(255,255,255,.15)}
.swFishing.phase-reel .swCastBar:before{left:var(--catch-zone-left,40%);right:auto;width:var(--catch-zone-width,22%);top:3px;bottom:3px;background:linear-gradient(90deg,rgba(34,197,94,.12),rgba(236,253,245,.64),rgba(34,197,94,.12));border:2px solid rgba(240,253,244,.95);box-shadow:0 0 18px rgba(34,197,94,.75),inset 0 0 14px rgba(255,255,255,.30);opacity:1}
.swFishing.phase-reel .swCastBar i{position:relative;z-index:2;background:linear-gradient(90deg,rgba(56,189,248,.75),rgba(59,130,246,.92));box-shadow:inset -2px 0 0 rgba(255,255,255,.65),0 0 12px rgba(56,189,248,.28)}
.swFishing.phase-reel .swCastBar em{z-index:4;background:#facc15;box-shadow:0 0 0 2px rgba(20,15,5,.72),0 0 18px rgba(250,204,21,.95)}
.swFishing.phase-reel .swCastBar em:after{content:'fish';position:absolute;left:50%;top:-20px;transform:translateX(-50%);font:900 10px Arial,sans-serif;color:#fde68a;text-shadow:0 2px 5px #000;letter-spacing:.04em;text-transform:uppercase}
.swFishing.phase-reel .swFishControls:after{content:'  ·  Blue bar = your line tension';color:#e0f2fe}
.swFishing.phase-reel #swFishHint{color:#fef3c7}
