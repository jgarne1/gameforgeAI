...
(function(){
  ...
  var engine={mounted:false,running:false,root:null,viewport:null,world:null,playerEl:null,shadowEl:null,fx:null,ui:null,transitionEl:null,scale:1,offX:0,offY:0,
    ...
    username:'guest',fishStats:{total:0,last:'None'},sceneId:'shadow_woods_dock',requestedSpawnId:'',sceneRegistry:null};
  ...
  function mount(){
    ...
    root.innerHTML=''
      ...
      +    '<div class="swFishing swSmartHud hidden" id="swFishing"><h3>Fishing</h3><p id="swFishText">Hold Space or mouse to cast farther.</p><div class="swCastBar"><i id="swCastFill"></i><em id="swSweet"></em></div><small id="swFishHint">Release to cast</small></div>'
      +  '</div>'
      +'</div>';
  ...
  function updateFishing(dt){
    ...
    if(f.phase==='walk'){
      ...
      f.t+=dt;var diff=Number(f.difficulty||1);f.fishPos=0.5+Math.sin(f.t*(2.7+diff*.55))*0.24+Math.sin(f.t*(6.1+diff*.9))*0.06;f.fishPos=Math.max(.08,Math.min(.92,f.fishPos));
      if(engine.mouseDown||engine.keys.Space||engine.keys.KeyE)f.tension+=dt*(0.34+diff*.035);else f.tension-=dt*(0.21+diff*.02);f.tension=Math.max(0,Math.min(1,f.tension));
      var window=Math.max(.12,.22-diff*.025);var good=Math.abs(f.tension-f.fishPos)<window;f.progress+=dt*(good?(0.28+Math.max(0,1.8-diff)*.045):-(0.08+diff*.035));f.progress=Math.max(0,Math.min(1,f.progress));
      showFishing(good?'Good tension — keep it in the bright zone!':'Hold/release to chase the fish marker.','Catch progress '+Math.round(f.progress*100)+'%');updateFishHud();
  ...
  function startFishing(h){if(engine.mode!=='explore')return;if(h)applyFishingHotspot(h);engine.mode='fish';var target=chooseFishTarget(h);engine.fish={phase:'walk',target:target,difficulty:target.difficulty||1,power:0,t:0,biteAt:1.0+Math.random()*1.25,tension:0.5,fishPos:0.5,progress:0,aim:null,preview:null,bobberX:FISH_TARGET.x,bobberY:FISH_TARGET.y};toast('Moving to the casting spot...');showFishing('Moving to the casting spot...','Esc cancels fishing');updateFishHud();engine.ui.classList.remove('hidden');}
  ...
  function catchFish(){var f=engine.fish||{};var target=f.target||chooseFishTarget(engine.activeHotspot);if(engine.fish)engine.fish.phase='catch';toast('Caught '+target.name+'! Added to Inventory.');showFishing('Caught '+target.name+'!','Saved in Inventory → Fishing. Cast again in a moment.');saveFishCatch(target);setTimeout(function(){if(engine.mode==='fish'){engine.fish={phase:'ready',target:chooseFishTarget(engine.activeHotspot),difficulty:1,power:0,t:0,biteAt:1.0+Math.random()*1.25,tension:0.5,fishPos:0.5,progress:0,aim:null,preview:null,bobberX:FISH_TARGET.x,bobberY:FISH_TARGET.y};showFishing('Ready for another cast.','Hold Space/click, aim into the water, then release.');updateFishHud();}},1050);}
  ...
  function chooseFishTarget(h){var table=(h&&Array.isArray(h.fishTable)&&h.fishTable.length)?h.fishTable:null;if(!table){table=[{id:'fish_drift_minnow',name:'Drift Minnow',rarity:'common',difficulty:1,weight:46},{id:'fish_bubble_guppy',name:'Bubble Guppy',rarity:'common',difficulty:1,weight:34},{id:'fish_moon_anchovy',name:'Moon Anchovy',rarity:'common',difficulty:1.1,weight:24},{id:'fish_glowfin',name:'Glowfin',rarity:'uncommon',difficulty:1.45,weight:14},{id:'fish_lantern_koi',name:'Lantern Koi',rarity:'rare',difficulty:1.9,weight:6},{id:'fish_ancient_coelafish',name:'Ancient Coelafish',rarity:'legendary',difficulty:2.65,weight:1}];}
  ...
  function normalizeFishTarget(x){x=x||{};return {id:x.id||x.itemId||'fish_drift_minnow',name:x.name||x.label||'Drift Minnow',rarity:x.rarity||'common',difficulty:Number(x.difficulty||1)};}
  function updateFishHud(){var c=document.getElementById('swFishCount');if(c)c.textContent=(engine.fishStats.total||0)+' fish';var cf=document.getElementById('swCatchFill');if(cf)cf.style.width=Math.round(((engine.fish&&engine.fish.progress)||0)*100)+'%';}
  async function loadFishingStats(){try{var res=await fetch('/api/pet/profile?user='+encodeURIComponent(engine.username),{cache:'no-store'});var data=await res.json();var inv=(data.profile&&data.profile.inventory)||{};var total=Object.keys(inv).filter(function(id){return id.indexOf('fish_')===0;}).reduce(function(sum,id){return sum+Number(inv[id]||0);},0);engine.fishStats.total=total;updateFishHud();}catch(e){}}
  async function saveFishCatch(target){engine.fishStats.total=Number(engine.fishStats.total||0)+1;engine.fishStats.last=target.name;updateFishHud();try{var res=await fetch('/api/pet/fish/catch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:engine.username,itemId:target.id,name:target.name,rarity:target.rarity,sceneId:engine.sceneId})});var data=await res.json();if(data&&data.profile){var inv=data.profile.inventory||{};engine.fishStats.total=Object.keys(inv).filter(function(id){return id.indexOf('fish_')===0;}).reduce(function(sum,id){return sum+Number(inv[id]||0);},0);updateFishHud();}}catch(e){toast('Caught '+target.name+', but inventory sync failed.');}}
