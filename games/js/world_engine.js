...
(function(){
  ...
  function render(dt){
    ...
    engine.shadowEl.style.transform=anim.walk?('scale('+ (1+Math.abs(Math.sin(s.animTime*Math.PI))*.10).toFixed(2)+')'):'scale(1)';
  ...
  function getAnim(){
    ...
    if(engine.mode==='fish'){
      if(f&&f.phase==='walk')return {key:'walk_right',fps:8,frames:frames(6,6),flip:true,walk:true};
      ...
      return {key:'fish_idle_right',fps:3,frames:frames(8,4)};
    }
    if(st.moving){
      ...
      if(st.face==='left')return {key:'walk_left',fps:8,frames:frames(6,6),walk:true};
      return {key:'walk_right',fps:8,frames:frames(6,6),flip:true,walk:true};
    }
    if(st.face==='up')return {key:'idle_up',fps:1,frames:frames(1,1)};
    if(st.face==='left')return {key:'idle_left',fps:1,frames:frames(2,1)};
    if(st.face==='right')return {key:'idle_right',fps:1,frames:frames(2,1),flip:true};
