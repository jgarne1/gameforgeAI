...
function key(ev){const down=ev.type==='keydown',k=ev.key.toLowerCase();if(['arrowup','arrowdown','arrowleft','arrowright','w','a','s','d','e','b','escape'].includes(k)){if(k==='e'&&down){interact();ev.preventDefault();return}if(k==='b'&&down){E.debug=!E.debug;const b=$('#wfDebug');if(b)b.classList.toggle('on',E.debug);return}if(k==='escape'&&down){hideHomeBox();return}E.keys[k]=down;ev.preventDefault()}}
...
async function loadScene(id,spawn){
  ...
  try{
    ...
    const imgs=[];(sc.objects||[]).forEach(o=>{const a=E.catalog[o.asset];if(a&&a.src)imgs.push(loadImg(a.src))});if(sc.playerSprite)imgs.push(loadImg(sc.playerSprite));await Promise.all(imgs);
...
function canStand(x,y){const sc=E.scene;if(!sc)return true;const w=sc.size?.w||1700,h=sc.size?.h||1200;if(x<8||y<8||x>w-8||y>h-8)return false;for(const o of sc.objects||[]){if(!o.collide)continue;const c=o.collide,s=o.scale||1,rx=o.x+c[0]*s,ry=o.y+c[1]*s,rw=c[2]*s,rh=c[3]*s;if(x>rx&&x<rx+rw&&y>ry&&y<ry+rh)return false}return true}
...
function drawBase(c,sc,time){if(sc.mode==='interior'){const it=sc.interior||{};c.fillStyle=it.wall||'#2c211c';c.fillRect(0,0,sc.size.w,sc.size.h);c.fillStyle=it.floor||'#60422c';roundRect(c,80,130,sc.size.w-160,sc.size.h-190,24,true,false);c.strokeStyle=it.trim||'#d1a75d';c.lineWidth=10;c.stroke();c.fillStyle=it.rug||'#6d3340';roundRect(c,sc.size.w/2-160,sc.size.h/2-40,320,150,18,true,false);return}const w=sc.size?.w||1700,h=sc.size?.h||1200,g=c.createLinearGradient(0,0,w,h);g.addColorStop(0,'#1d3e28');g.addColorStop(.5,'#356b37');g.addColorStop(1,'#1a3827');c.fillStyle=g;c.fillRect(0,0,w,h);drawTerraces(c,sc);drawWater(c,sc,time);drawPaths(c,sc,time);drawGroundDetails(c,sc)}
...
function drawPlayer(c,time){c.save();c.translate(E.player.x,E.player.y);c.fillStyle='rgba(0,0,0,.34)';c.beginPath();c.ellipse(0,10,20,8,0,0,7);c.fill();const sheet=E.scene?.playerSprite&&E.assets[E.scene.playerSprite];if(sheet&&sheet.width>=512&&sheet.height>=512){const fw=128,fh=128;let row=0;if(E.player.face==='up')row=1;else if(E.player.face==='left')row=2;else if(E.player.face==='right')row=3;if(E.player.moving)row+=4;const col=E.player.moving?(Math.floor(time*9)%6):(Math.floor(time*2)%4);c.drawImage(sheet,col*fw,row*fh,fw,fh,-48,-105,96,96)}else{c.fillStyle='#4ba3ff';c.strokeStyle='#062033';c.lineWidth=3;roundRect(c,-16,-48,32,52,11,true,true);c.fillStyle='#ffe4b8';c.beginPath();c.arc(0,-57,14,0,7);c.fill()}label(c,E.displayName||E.username,0,-88);c.restore()}
