...
(function(){
  ...
  var engine={mounted:false,running:false,root:null,viewport:null,world:null,playerEl:null,shadowEl:null,fx:null,ui:null,transitionEl:null,scale:1,offX:0,offY:0,
    keys:{}, mouseDown:false, state:null, target:null, mode:'explore', fish:null, activeHotspot:null, raf:0, last:0, onClose:null,debug:false,transitioning:false,
  ...
  function updateFishing(dt){
    ...
    if(f.phase==='walk'){
      ...
      else{f.phase='ready';f.t=0;showFishing('Ready to fish. Press Space or click to start your cast.','Esc cancels fishing');toast('Fishing spot ready. Press Space or click to cast.');}
    }
      ...
      f.t+=dt;if(f.t>1.15){f.phase='wait';f.t=0;f.biteAt=1+Math.random()*1.4;showFishing('Missed it. Keep watching the bobber.','Press only when it bites');}
      ...
      showFishing('Reeling: hold/release Space to keep the bar near the marker.','Progress '+Math.round(f.progress*100)+'%');
  ...
  function startFishing(h){if(engine.mode!=='explore')return;if(h)applyFishingHotspot(h);engine.mode='fish';engine.fish={phase:'walk',power:0,t:0,biteAt:1.2+Math.random()*1.4,tension:0.5,fishPos:0.5,progress:0};toast('Moving to the casting spot...');showFishing('Moving to the casting spot...','Esc cancels fishing');engine.ui.classList.remove('hidden');}
  ...
  function beginCastCharge(){var f=engine.fish;if(!f||f.phase!=='ready')return;f.phase='aim';f.power=0;f.t=0;showFishing('Charging cast... release Space/click to throw.','Aim for the green-gold sweet spot');}
