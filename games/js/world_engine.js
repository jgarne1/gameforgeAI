/*
  GameForge AI World Engine - Shadow Woods vertical slice v1.1
...
(function(){
  ...
  var SPRITE={cols:8,rows:12,w:128,h:128,naturalW:1024,naturalH:1536,loaded:false};
  ...
  function render(dt){
    ...
    var sprite=engine.playerEl.querySelector('.swSprite');var anim=getAnim();if(s.animKey!==anim.key){s.animKey=anim.key;s.animTime=0;}s.animTime+=dt*(anim.fps||6);var idx=Math.floor(s.animTime)%anim.frames.length;var fr=anim.frames[idx];sprite.style.backgroundImage='url('+ASSETS.player+')';sprite.style.backgroundSize=SPRITE.naturalW+'px '+SPRITE.naturalH+'px';sprite.style.backgroundPosition=(-fr[0]*SPRITE.w)+'px '+(-fr[1]*SPRITE.h)+'px';
  ...
  function getAnim(){
    ...
    if(engine.mode==='fish'){
      if(f&&f.phase==='walk')return {key:'walk_right',fps:8,frames:frames(7,6)};
      if(f&&f.phase==='aim')return {key:'fish_cast_right',fps:5,frames:frames(9,6)};
      if(f&&f.phase==='reel')return {key:'fish_reel_right',fps:7,frames:frames(10,6)};
      if(f&&f.phase==='catch')return {key:'fish_catch_right',fps:6,frames:frames(11,4)};
      return {key:'fish_idle_right',fps:4,frames:frames(8,4)};
    }
    if(st.moving){
      if(st.face==='down')return {key:'walk_down',fps:8,frames:frames(4,6)};
      if(st.face==='up')return {key:'walk_up',fps:8,frames:frames(5,6)};
      if(st.face==='left')return {key:'walk_left',fps:8,frames:frames(6,6)};
      return {key:'walk_right',fps:8,frames:frames(7,6)};
    }
    if(st.face==='up')return {key:'idle_up',fps:4,frames:frames(1,4)};
    if(st.face==='left')return {key:'idle_left',fps:4,frames:frames(2,4)};
    if(st.face==='right')return {key:'idle_right',fps:4,frames:frames(3,4)};
    return {key:'idle_down',fps:4,frames:frames(0,4)};
  }
  ...
  function preloadSpriteSheet(){
    ...
    img.onload=function(){
      ...
      SPRITE.w=Math.round(SPRITE.naturalH/12);SPRITE.h=SPRITE.w;SPRITE.cols=Math.max(1,Math.round(SPRITE.naturalW/SPRITE.w));SPRITE.rows=12;SPRITE.loaded=true;
    };
