...
const estateNeighborhoodsFile=path.join(DATA,'estate_neighborhoods.json');
...
function defaultAnnouncement(){
  return {
    ...
    message:'Battle Hall is live. Sit at a table, challenge a friend, or check back here for updates from the GameForge team.',
...
function normalizeAnnouncement(raw){
  ...
  let fallback=defaultAnnouncement();
  let action=String(raw.buttonAction||fallback.buttonAction).trim().toLowerCase();
  let allowed=['none','battlehall','privatebattle','petworld','marketplace','play','social'];
  if(!allowed.includes(action))action='none';
  return {
    active:raw.active!==false,
    title:String(raw.title||fallback.title).trim().slice(0,70)||fallback.title,
    message:String(raw.message||fallback.message).trim().slice(0,500)||fallback.message,
    signature:String(raw.signature||fallback.signature).trim().slice(0,40)||fallback.signature,
    buttonText:String(raw.buttonText||fallback.buttonText).trim().slice(0,30)||fallback.buttonText,
    buttonAction:action,
...
function announcement(){
  return normalizeAnnouncement(readJSON(announcementFile,defaultAnnouncement()));
}
...
app.post('/api/admin/announcement',(req,res)=>{
  ...
  if(!adminUser)return;
  let saved=saveAnnouncement({
    active:req.body.active!==false,
    title:req.body.title,
    message:req.body.message,
    signature:req.body.signature,
    buttonText:req.body.buttonText,
    buttonAction:req.body.buttonAction,
    updatedAt:Date.now(),
    updatedBy:adminUser
  });
  res.json({ok:true,message:'Home announcement updated.',announcement:saved});
});
...
app.post('/api/admin/world-forger/delete',(req,res)=>{
  let adminUser=requireAdmin(req,res,'full_admin');
  if(!adminUser)return;
  try{
    const sceneId=String((req.body&&req.body.sceneId)||'').replace(/[^a-zA-Z0-9_-]/g,'');
    ...
    const target=path.join(sceneDir,sceneId+'.json');
    if(!target.startsWith(sceneDir))return res.status(400).json({ok:false,error:'Invalid scene path'});
    if(!fs.existsSync(target))return res.status(404).json({ok:false,error:'Scene not found'});
    const trashDir=path.join(sceneDir,'_deleted');
    fs.mkdirSync(trashDir,{recursive:true});
    const stamp=new Date().toISOString().replace(/[:.]/g,'-');
    const backup=path.join(trashDir,sceneId+'.'+stamp+'.json');
    fs.renameSync(target,backup);
    res.json({ok:true,deleted:sceneId,backup:'/assets/worlds/_deleted/'+path.basename(backup),deletedBy:adminUser,updatedAt:Date.now()});
  }catch(err){
    res.status(500).json({ok:false,error:err.message||'Could not delete world scene'});
  }
});
