...
- Preserve the account, room, WebSocket, iframe game, chat, marketplace, and pet APIs unless the user explicitly asks to redesign them.
...
- Do not trust client-provided pet objects for ownership transfers, rewards, training, exploration, or battle payouts.
- Marketplace listings escrow items on the server when listed; never let buyers/sellers transfer items only on the client.
- Multiplayer room gameState is in-memory for reconnect/late-join support. Games should send {type:'state',state} after accepted moves.
*/
